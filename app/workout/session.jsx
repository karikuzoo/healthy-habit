...
import { Button, Card, ExerciseMedia, Screen, ScreenHeader } from '../../src/components';
        ...
        <Card className="items-center gap-3 p-6">
          ...
          {equipmentLabel(exercise) ? (
            <View className="flex-row items-center gap-1.5 rounded-full bg-steps-soft px-3 py-1">
              <Ionicons name="alert-circle-outline" size={13} color={colors.steps.DEFAULT} />
              <Text className="text-xs font-semibold text-steps">{equipmentLabel(exercise)}</Text>
            </View>
          ) : null}
